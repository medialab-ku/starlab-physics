...
ti.root.dense(ti.i, num_particles).place(Aii, src, Ax, res, tmp)
...
def update_lambdas(omega: float):
    for p_i in positions:
        ...
        p[p_i] += omega * res[p_i]  / (Aii[p_i] + 1e-3)
...
def substep():
    ...
    for p_i in positions:
        ...
        for j in range(particle_num_neighbors[p_i]):
            ...
            a_i -= (lambda_i + lambda_j) * spiky_gradient(pos_ji, h_)
        # pos_delta_i /= rho0
        velocities[p_i] += time_delta * a_i
    # apply position deltas
    for i in positions:
        positions[i] += time_delta * velocities[i]
@ti.kernel
def epilogue():
    # confine to boundary
    for i in positions:
...
def precompute():
    ...
    for p_i in positions:
        ...
        for j in range(particle_num_neighbors[p_i]):
            ...
            div += grad_j.dot(velocities[p_i] - velocities[p_j])
            grad_i += grad_j
            sum_gradient_sqr += grad_j.dot(grad_j)
            density += poly6_value(pos_ji.norm(), h_)
        sum_gradient_sqr += grad_i.dot(grad_i)
        Aii[p_i] = sum_gradient_sqr
        src[p_i] = (density + time_delta * div - 1.0) / (time_delta ** 2)

@ti.kernel
def compute_Ax(p: ti.template()):
    ...
    for p_i in positions:
        ...
        for j in range(particle_num_neighbors[p_i]):
            ...
            if p_j < 0:
                break
            tmp[p_i] += (p[p_i] + p[p_j]) * spiky_gradient(old_positions[p_i] - old_positions[p_j], h_)
    ...
    for p_i in positions:
        ...
        for j in range(particle_num_neighbors[p_i]):
            ...
            if p_j < 0:
                break
            Ax[p_i] += (tmp[p_i] - tmp[p_j]).dot( spiky_gradient(old_positions[p_i] - old_positions[p_j], h_))
            # Ax[p_i] += lambdas[p_j] * spiky_gradient(positions[p_i] - positions[p_j], h_)
...
def run_pbf():
    ...
    for _ in range(max_jacobi_iter):
        ...
        # res.fill(0.0)
        # add(res, res, time_delta ** 2, src)
        # add(res, res, -time_delta ** 2, Ax)

        update_lambdas(omega=0.5)
