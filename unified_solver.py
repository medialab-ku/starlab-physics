...
class UnifiedSolver:
    def __init__(self, particle_system, elasticity, pressure):
        ...
        self.dv     = ti.Vector.field(n=3, dtype=float, shape=self.ps.particle_max_num)
    ...
    @ti.kernel
    def set_rest_volume_and_L(self):
    ...
    def precompute_value(self):
        ...
        for p_i in ti.grouped(self.ps.x):
            ...
            for j in range(self.ps.solid_neighbors_num[p_i0]):
                ...
                self.K[n] = self.ps.m_V0[p_i] * self.ps.m_V0[p_j] * self.W(xij0.norm(),
                                                                           self.ps.support_radius) / xij0.norm_sqr()
    ...
    @ti.kernel
    def compute_P(self, YM: float, PR: float) -> float:
        ...
        for p_i in ti.grouped(self.ps.x):
            ...
            energy += mu * self.ps.m_V0[p_i] * (F_i - R_i).norm_sqr()
    ...
    def solve(self, dt):
        ...
        self.stretch_energy = self.compute_P(YM, PR)
        ...
        self.stats_stretch_energy.append(float(self.stretch_energy))
    ...
    def clear_stats(self):
        ...
        self.stats_stretch_energy.clear()
        self.stats_elapsed_ms.clear()
    ...
    def get_stats_numpy(self):
        ...
        return {
            ...
            "stretch_energy": np.asarray(self.stats_stretch_energy, dtype=np.float64)
        }