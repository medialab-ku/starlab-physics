...
class DFSPHSolver(SPHBase):
    ...
    def compute_DFSPH_factor(self):
        ...
        for p_i in ti.grouped(self.ps.x):
            ...
            m_i = self.density_0 * self.ps.m_V0[p_i]
            ...
            self.ps.dfsph_factor[p_i] = 1.0 / schur

    @ti.func
    def compute_DFSPH_factor_task(self, p_i, p_j, ret: ti.template()):
        if self.ps.material[p_j] == self.ps.material_fluid:
            # Fluid neighbors
            m_j = self.density_0 * self.ps.m_V0[p_j]
            ...
            # Boundary neighbors
            ## Akinci2012
            grad_p_j = -self.ps.m_V0[p_j] * self.cubic_kernel_derivative(self.ps.x[p_i] - self.ps.x[p_j])
            for i in ti.static(range(3)):  # grad_p_i
                ret[i] -= grad_p_j[i]
    ...
    def compute_density_change_task(self, p_i, p_j, ret: ti.template()):
        ...
        if self.ps.material[p_j] == self.ps.material_fluid:
            # Fluid neighbors
            ret.density_adv += self.ps.m_V0[p_j] * (v_i - v_j).dot(
    ...
    def compute_density_adv_task(self, p_i, p_j, ret: ti.template()):
        ...
        if self.ps.material[p_j] == self.ps.material_fluid:
            ...
            # Boundary neighbors
            ## Akinci2012
            ret += self.ps.m_V0[p_j] * (v_i - v_j).dot(self.cubic_kernel_derivative(self.ps.x[p_i] - self.ps.x[p_j]))
    ...
    def compute_density_error(self, offset: float) -> float:
        ...
        for I in ti.grouped(self.ps.x):
            if self.ps.material[I] == self.ps.material_fluid:
                density_error += ti.max(self.ps.density_adv[I] - offset, 0.0)
    ...
    def compute_lambda(self):
        ...
        for p_i in ti.grouped(self.ps.x):
            if self.ps.material[p_i] != self.ps.material_fluid:
                continue
            # Evaluate rhs
            c_i = ti.max(self.ps.density_adv[p_i] - self.density_0, 0.0)
            self.ps.pressure[p_i] = -c_i * self.ps.dfsph_factor[p_i]
    ...
    def update_velocity_task(self, p_i, p_j, dv: ti.template()):
        if self.ps.material[p_j] == self.ps.material_fluid:
            ...
            # if ti.abs(k_sum) > self.m_eps:
            m_j = self.density_0 * self.ps.m_V0[p_j]
            m_i = self.density_0 * self.ps.m_V0[p_j]
            grad_p_j = m_j * self.cubic_kernel_derivative(self.ps.x[p_i] - self.ps.x[p_j])
